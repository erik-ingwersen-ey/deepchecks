...
class ReduceFeatureMixin(ReduceMixin):
    ...
    def feature_reduce(aggregation_method: str, value_per_feature: pd.Series, feature_importance: Optional[np.array],
        ...
        if aggregation_method == 'max':
            return {str(f'Max {score_name}'): np.max(value_per_feature)}

        elif aggregation_method == 'mean':
            return {str(f'Mean {score_name}'): np.mean(value_per_feature)}
        elif aggregation_method == 'none':
            return dict(value_per_feature)
        if (
            aggregation_method in {'weighted', 'l2_weighted', 'top_5'}
            ...
            return {
                str(f'Weighted {score_name}'): np.sum(
                    np.array(value_per_feature) * feature_importance
                )
            }
        elif aggregation_method == 'l2_weighted':
            sum_drift_fi = np.array(value_per_feature) + feature_importance
            return {
                str(f'L2 Weighted {score_name}'): np.linalg.norm(sum_drift_fi)
                - np.linalg.norm(feature_importance)
            }
        else:
...
class ReducePropertyMixin(ReduceMixin):
    ...
    def property_reduce(aggregation_method: str, value_per_property: pd.Series, score_name: str) -> Dict[str, float]:
        """Return an aggregated drift score based on aggregation method defined."""
        if aggregation_method == 'max':
            return {str(f'Max {score_name}'): np.max(value_per_property)}
        elif aggregation_method == 'mean':
            return {str(f'Mean {score_name}'): np.mean(value_per_property)}
        elif aggregation_method == 'none':
