...
def _calc_recall(tp: float, fp: float, fn: float) -> float:    # pylint: disable=unused-argument
    ...
    return -1 if tp + fn == 0 else tp / (tp + fn + np.finfo(float).eps)
...
def _calc_precision(tp: float, fp: float, fn: float) -> float:
    ...
    if tp + fn == 0:
        return -1
    return 0 if tp + fp == 0 else tp / (tp + fp + np.finfo(float).eps)
...
def _calc_f1(tp: float, fp: float, fn: float) -> float:
    ...
    return (2 * rc * pr) / (rc + pr + np.finfo(float).eps)
...
def _calc_fpr(tp: float, fp: float, fn: float) -> float:
    ...
    if tp + fn == 0:
        return -1
    return 0 if tp + fp == 0 else fp / (tp + fn + np.finfo(float).eps)
...
def _calc_fnr(tp: float, fp: float, fn: float) -> float:
    ...
    if tp + fn == 0:
        return -1
    return 1 if tp + fp == 0 else fn / (tp + fn + np.finfo(float).eps)
